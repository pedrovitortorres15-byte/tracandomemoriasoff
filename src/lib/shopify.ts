...
const SHOPIFY_STORE_PERMANENT_DOMAIN = 'mugs-memories-studio-dfnrz.myshopify.com';
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = 'd4433ad77861a68a12e8b1e9ec382d9e';
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (response.status === 402) {
    toast.error("Shopify: Pagamento necessário", {
      description: "A API do Shopify requer um plano de faturamento ativo.",
    });
    return;
  }
